...
<h3> Review Join Seller Join Product Table </h3>
...
$total_pages_sql = "SELECT COUNT(*) FROM review natural join seller natural join product";
$result = mysqli_query($conn,$total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

$sql = "SELECT * FROM review natural join seller natural join product LIMIT $offset, $no_of_records_per_page";
...
while ($row = mysqli_fetch_array($res_data)){
    ...
    $field4name = $row["s_id"];
    $field5name = $row["s_fname"];
    $field6name = $row["s_lname"];
    $field7name = $row["s_site"]; 
    $field8name = $row["prod_code"];
    $field9name = $row["prod_price"];
    $field10name = $row["prod_type"];
    $field11name = $row["d_id"];
    $field12name = $row["cat_id"];
    ...
    echo "First Name: ".$field5name.'<br />';
    echo "Last Name: ".$field6name.'<br />';
    echo "Seller's Site: ".$field7name.'<br />';
    echo "Product Code: ".$field8name.'<br />';
    echo "Product Price: $".$field9name.'<br />';
    echo "Product Type: ".$field10name.'<br />';
    echo "Designer ID: ".$field11name.'<br />';
    echo "Category ID: ".$field12name.'<br />';
    echo '</b>';
    echo '<br />';
}
mysqli_close($conn);
