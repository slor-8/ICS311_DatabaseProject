...
<h3> Seller Join Designer/Company Table </h3>
...
$total_pages_sql = "SELECT COUNT(*) FROM seller natural join designerComp";
$result = mysqli_query($conn,$total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

$sql = "SELECT * FROM seller natural join designerComp LIMIT $offset, $no_of_records_per_page";
$res_data = mysqli_query($conn,$sql);
while ($row = mysqli_fetch_array($res_data)){
    $field1name = $row["s_id"];
    $field2name = $row["s_fname"];
    $field3name = $row["s_lname"];
    $field4name = $row["s_site"];
    $field5name = $row["d_id"];
    $field6name = $row["d_loc"];
    $field7name = $row["d_name"];

    echo '<b>'."Seller's ID: ".$field1name.'<br />';
    echo "First Name: ".$field2name.'<br />';
    echo "Last Name: ".$field3name.'<br />';
    echo "Seller's Site: ".$field4name.'<br />';
    echo "Designer ID: ".$field5name.'<br />';
    echo "Designer Location: ".$field6name.'<br />';
    echo "Designer Name: ".$field7name.'<br />';
    echo '</b>';
    echo '<br />';
}
mysqli_close($conn);
