...
<h3> Product Join Category Table </h3>
...
$total_pages_sql = "SELECT COUNT(*) FROM product natural join category";
$result = mysqli_query($conn,$total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

$sql = "SELECT * FROM product natural join category LIMIT $offset, $no_of_records_per_page";
$res_data = mysqli_query($conn,$sql);
while ($row = mysqli_fetch_array($res_data)){
    $field1name = $row["prod_code"];
    $field2name = $row["prod_price"];
    $field3name = $row["prod_type"];
    $field4name = $row["d_id"];
    $field5name = $row["cat_id"];
    $field6name = $row["cat_style"];
    $field7name = $row["cat_color"];
    $field8name = $row["cat_sew_technique"];
    $field9name = $row["cat_set"];
    $field10name = $row["cat_size"];

    echo '<b>'."Product Code: ".$field1name.'<br />';
    echo "Product Price: $".$field2name.'<br />';
    echo "Product Type: ".$field3name.'<br />';
    echo "Designer ID: ".$field4name.'<br />';
    echo "Category ID: ".$field5name.'<br />';
    echo "Style: ".$field6name.'<br />';
    echo "Color: ".$field7name.'<br />';
    echo "Sewing Technique: ".$field8name.'<br />';
    echo "In A Set: ".$field9name.'<br />';
    echo "size: ".$field10name.'<br />';
    echo '</b>';
    echo '<br />';
}
mysqli_close($conn);
