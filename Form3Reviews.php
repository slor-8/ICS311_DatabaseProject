...
<!-- user input -->
<form action="Form3Reviews.php" method="post">
Search <input type="text" name="search"><br>

Column: <select name="column">
<option value="rev_code">Review Code</option>
	<option value="rev_name">Name</option>
    <option value="rev_rate">Rate</option>
...
$sql = "select * from review where $column like '%$search%'";
...
while($row = $result->fetch_assoc() ){
    ...
    echo '<b>'."Review Code: ".$row["rev_code"].'<br />';
    echo "Reviewer's Name: ".$row["rev_name"].'<br />';
    echo "Rate: ".$row["rev_rate"].'<br />';
    echo "Seller ID: ".$row["s_id"].'<br />';
    echo "Product Code: ".$row["prod_code"].'<br />';
    echo '</b>';
}
} else {
	echo "0 records";
