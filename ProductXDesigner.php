...
<h3> Product Join Designer/Company Table </h3>
...
$total_pages_sql = "SELECT COUNT(*) FROM product natural join designerComp";
$result = mysqli_query($conn,$total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

$sql = "SELECT * FROM product natural join designerComp LIMIT $offset, $no_of_records_per_page";
...
while ($row = mysqli_fetch_array($res_data)){
    ...
    $field4name = $row["d_id"];
    $field5name = $row["d_loc"];
    $field6name = $row["d_name"];
    $field7name = $row["cat_id"];
    ...
    echo "Designer Location: ".$field5name.'<br />';
    echo "Designer Name: ".$field6name.'<br />';
    echo "Category ID: ".$field7name.'<br />';
    echo '</b>';
    echo '<br />';
}
mysqli_close($conn);
