...
<!-- user input -->
<form action="Form3Category.php" method="post">
Search <input type="text" name="search"><br>

Column: <select name="column">
	<option value="cat_id">Category ID</option>
	<option value="cat_style">Style</option>
    <option value="cat_color">Color</option>
    <option value="cat_sew_technique">Sew Technique</option>
    <option value="cat_set">Set With</option>
    <option value="cat_size">Size</option>
	</select><br>
<input type ="submit">
</form>
...
$sql = "select * from category where $column like '%$search%'";
...
while($row = $result->fetch_assoc() ){
    ...
    echo '<b>'."Category ID: ".$row["cat_id"].'<br />';
    echo "Style: ".$row["cat_style"].'<br />';
    echo "Color: ".$row["cat_color"].'<br />';
    echo "Sewing Technique: ".$row["cat_sew_technique"].'<br />';
    echo "In A Set: ".$row["cat_set"].'<br />';
    echo "size: ".$row["cat_size"].'<br />';
    echo '</b>';
}
} else {
	echo "0 records";
