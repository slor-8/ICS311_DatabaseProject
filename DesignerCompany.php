...
$total_pages_sql = "SELECT COUNT(*) FROM designerComp";
$result = mysqli_query($conn,$total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

$sql = "SELECT * FROM designerComp LIMIT $offset, $no_of_records_per_page";
$res_data = mysqli_query($conn,$sql);
while ($row = mysqli_fetch_array($res_data)){
    $field1name = $row["d_id"];
    $field2name = $row["d_loc"];
    $field3name = $row["d_name"];

    echo '<b>'."Designer ID: ".$field1name.'<br />';
    echo "Designer Location: ".$field2name.'<br />';
    echo "Designer Name: ".$field3name.'<br />';
    echo '</b>';
}
mysqli_close($conn);
?>
